...
BASE_1 = readxl::read_xlsx(path = "C:/Users/nicol.NICOLAS_GP/Downloads/Base de datos.xlsx")

BASE_2 = readxl::read_xlsx(path = "C:/Users/nicol.NICOLAS_GP/Downloads/Base de datos.xlsx" , sheet = 2)

BASE_3 = readxl::read_xlsx(path = "C:/Users/nicol.NICOLAS_GP/Downloads/Base de datos.xlsx" , sheet = 3)


# Vamos a trabajar con la base n1
...
table(BASE_1$AÑO)
table(BASE_1$DEPARTAMENTO)

# Hacemos un bucle para iterar sobre las variables de tipo numericas
for (N in names(BASE_1)) {
  if (class(BASE_1[[N]]) == "numeric") {
    cat("\n📊 Variable:", N, "\n")
    cat("   Media:   ", mean( BASE_1[[N]] , na.rm = TRUE ), "\n") # Media
    cat("   Mediana: ", median( BASE_1[[N]], na.rm = TRUE)  , "\n") # Mediana
    cat("   Máximo:  ", max( BASE_1[[N]], na.rm = TRUE ), "\n") # Maximo
    cat("   Mínimo:  ", min(BASE_1[[N]], na.rm = TRUE ) , "\n") # Min
    cat("   Valores perdidos:", sum(is.na(BASE_1[[N]])) , "\n") # Num valores perdidos
  } 
}
 
# Recorremos cada columna del data frame
for (N in names(PERSONAS_GEIH)) {
  
  # Si la columna es numérica
  if (is.numeric(PERSONAS_GEIH[[N]])) {
    
    # Guardamos la columna en una variable temporal
    col_data <- PERSONAS_GEIH[[N]]
    
    # Calculamos las estadísticas
    media <- mean(col_data, na.rm = TRUE)       # media
    mediana <- median(col_data, na.rm = TRUE)   # mediana
    maximo <- max(col_data, na.rm = TRUE)       # máximo
    minimo <- min(col_data, na.rm = TRUE)       # mínimo
    perdidos <- sum(is.na(col_data))            # cantidad de NA
...
## Eliminar la variables Año

## Cree un nuevo objeto que se llame BASE_1_2 solo seleccionar el departamento y el PIB a precios constantes



#Git y GitHub con R
#https://happygitwithr.com/
#https://bookdown.org/drki_musa/dataanalysis/rstudio-project.html#dataset-repository-on-github
